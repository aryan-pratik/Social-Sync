        ...
        <section id="pricing" className="py-24 bg-slate-900">
            <div className="max-w-6xl mx-auto px-4 sm:px-6">
                <div className="text-center mb-16">
                    ...
                    <p className="mt-5 text-gray-400 max-w-md mx-auto">Start free, upgrade when you're ready. Cancel anytime — no hidden fees.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
                    {pricingPlans.map((plan) => (
                        <div key={plan.name} className={`rounded-2xl border p-7 flex flex-col gap-6 relative ${plan.highlight ? "bg-violet-500 text-white border-violet-400 shadow-2xl shadow-red-100" : "bg-slate-900 text-white border-white/10"}`}>
                            ...
                            <div>
                                ...
                                <p className={`text-sm mt-2 leading-relaxed ${plan.highlight ? "text-violet-100" : "text-slate-400"}`}>{plan.description}</p>
                            </div>
                            ...
                            <ul className="space-y-2.5">
                                {plan.features.map((f) => (
                                    <li key={f} className="flex items-center gap-2.5 text-sm">
                                        ...
                                        <span className={plan.highlight ? "text-violet-50" : "text-slate-600"}>{f}</span>
                                    </li>
                                ))}
                            </ul>

                            <Link to="/#" className={`mt-auto text-center font-semibold text-sm px-6 py-3 rounded-full ${plan.highlight ? "bg-slate-900 text-violet-500 hover:bg-violet-50" : "bg-violet-500 text-white hover:bg-violet-600"}`}>
                                {plan.cta}
                            </Link>
                        </div>
                    ))}
